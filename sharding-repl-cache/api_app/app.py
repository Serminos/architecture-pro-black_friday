...
# @cache(expire=60 * 1) Не работает. Поменял реализацию для проверки redis прямое использование
async def list_users(collection_name: str):
    ...
    if REDIS_URL:
        ...
        if cached:
            ...
            users = [UserModel(**item) for item in data]
    ...
    users_dicts = [UserModel(**u).dict(by_alias=True) for u in users]
    if REDIS_URL:
        await redis.setex(cache_key, 60, json.dumps(users_dicts))
    return UserCollection(users=users)

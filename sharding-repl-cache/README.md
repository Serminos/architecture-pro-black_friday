...
После полной инициализации кластера (шардирование + репликация) и запуска приложения выполните следующие шаги:

1. Откройте инструменты разработчика в браузере (F12) → вкладка **Network**.
2. Перейдите по адресу `http://localhost:8080/helloDoc/users`.
   - Первый запрос займёт около 1 секунды.
3. Обновите страницу (F5) несколько раз подряд.
   - **Время ответа должно стать менее 20 мс**, так как данные будут браться из Redis.
